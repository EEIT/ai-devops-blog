...
讀到這裡你可能會問:我用 Claude Code 就是一個人對著終端敲指令,哪來什麼「劇組」?這個問題正是這篇要回答的。你會發現,哪怕你是一個人在用,**Claude Code 內部的每次思考切換,其實都對應到某種「角色切換」**——你只是從來沒意識到這件事而已。沒意識到的代價,就是作品品質忽好忽壞,自己也說不清原因。

## 一個人當編劇、導演、演員、觀眾,會發生什麼?

讓我們往下挖一層。為什麼劇組一定要分工?為什麼不能一個人包辦所有角色?

答案不是「做不完」。一個有才華的創作者,確實可以自編自導自演自剪,獨立電影的歷史上不乏這樣的例子。**真正的問題在於,一個人同時扮演多個角色時,思維模式會互相汙染**。
...
這是一個生理層面的限制,不是意志力的問題。**認知模式的切換需要時間,而且切換不完全的時候,前一個模式的殘留會影響下一個判斷**。

現在把這個觀察搬到 AI 助理的世界。你給 Claude Code 一個任務:「幫我寫一個處理 CSV 的 Python 腳本,記得要有完整的錯誤處理和測試。」它開始工作。寫腳本時,它進入「建設者」模式——怎麼讓程式優雅、怎麼處理各種 edge case、怎麼結構化。寫完之後,你說:「幫我 review 一下這段程式碼。」它立刻切換成「審查者」模式,但它還記得自己剛才為什麼這樣寫,還會下意識地為自己的設計辯護。**結果就是那種典型的 AI 審查:每一行都寫「這段程式碼清楚地實現了...」,沒有真正的批判**。

我們人類有一個詞形容這件事,叫「**球員兼裁判**」。這不是罵人,是描述一種結構性的困境——同一個主體同時扮演相互制衡的角色,制衡就失效了。Claude Code 碰到的是完全一樣的困境。它是非常強的模型,但它也受限於同一次對話裡的認知慣性。

Agent Team 要解的,就是這個困境。做法出乎意料地簡單:**強制切換角色**。不是用意志力切換,是用系統設計切換。每個角色有自己的職責、自己的禁止清單、自己的思維模式定義;切換時有明確的儀式,讓前一個角色的殘留徹底清除;切換後用的是一份全新的、專屬這個角色的指令集。這不是在 prompt 上寫「現在你要扮演一個嚴格的 reviewer」就了事——那樣的「偽切換」沒用,因為沒有架構層級的強制力。真正的角色切換,像演員從劇本室走到片場,有空間位置的改變、有造型的改變、有整個氣氛的改變。
...
**第二根骨幹是「禁止操作清單」**。這是最多人忽略、但也最關鍵的一根。如果只有職責沒有禁止,角色會慢慢膨脹,最後每個角色都變成什麼都做的萬能 agent,分工就消失了。禁止清單要寫得像法律條文——不是「建議不要」而是「絕對不能」。例如 reviewer 的禁止清單可以寫:「不能直接修改被審查的程式碼、不能做架構決策、不能為被審查對象辯護」。這三句話乍看很嚴格,但正是這種嚴格,讓 reviewer 保持了「審查者」的獨立性。
...
有了這四根骨幹,一個 Agent Team 才算完整。缺一不可。少了角色定義,等於沒有演員表;少了禁止清單,等於沒有合約條款;少了交接協定,等於劇組沒有場記;少了狀態檔案,等於沒有拍攝進度表。這四樣東西組合起來,才是一個可以穩定運作的工作系統,而不只是一個「比較聰明的 AI」。

## 三個起手式角色:用最少的分工做最多的事

你可能會擔心:那我是不是要設計一堆角色才能用 Agent Team?不是。事實上,大多數場景用**三個角色**就綽綽有餘。這三個角色是:spec-writer、implementer、reviewer。
...
**reviewer 是那個「假設每個地方都有問題,直到證明沒有」的角色**。它的思維模式是「懷疑主義」。這個角色的存在價值就是不信任——不信任 spec 已經想周全了、不信任 implementer 沒有偷懶、不信任程式碼沒有漏洞。它要找的是 bug、是 edge case、是被忽略的 security issue、是 spec 跟實作之間的 gap。

reviewer 的禁止清單必須包含最關鍵的一條:**不能修改被審查的程式碼**。這一條可能是整個 Agent Team 設計裡面最重要的一條規則。為什麼?因為如果 reviewer 可以直接改程式碼,它的本質就從「審查者」變成「另一個實作者」。它會不自覺地想「算了我自己改比較快」,然後就回到「球員兼裁判」的老問題了。reviewer 能做的只有兩件事:**標記問題、退回給 implementer**。

這三個角色的組合,為什麼夠用?因為它覆蓋了工程工作最基本的三個階段:**問清楚(spec)、做出來(implement)、驗對了(review)**。任何軟體工作,剝到最底層,都是這三件事的反覆。

當然你可以加更多角色——架構師、資安稽核、DBA、技術文件撰寫者、專案經理——每個角色解決特定場景。但如果你是第一次嘗試 Agent Team,從這三個角色開始最穩。先讓這三個角色順暢運作一兩週,再慢慢擴充。一開始就堆出十個角色,絕對會亂。
...
如果在下指令之前,有另一個人——一個 reviewer 角色——看一眼他打算做的操作,事情可能完全不會發生。不是因為那個人技術更好,而是因為 reviewer 的**思維模式本來就是懷疑主義**——一看到 `cleanse` 這個字就會問:「你確定要做這個?它的副作用是什麼?有沒有其他解法?」這個問句,可能救回那 100GB 資料。

現在把這個故事轉到 AI 的場景。如果你給 Claude Code 一個運維任務,它進入「建設者」模式試圖解決問題。當它試了三四種方法都失敗,它可能會開始「模仿人類緊急應變的壓力反應」——提出越來越激進的方案。這時候如果有另一個角色(reviewer)存在,即使它在同一個 AI 背後,它的 prompt 強迫它用懷疑主義看待眼前的提案,它會攔下這個激進決定。

這就是 Agent Team 真正的價值——不是讓 AI 做得更快,是讓它做得**更慢、更謹慎、更可逆**。在權力放大器(能執行 bash 的 AI)的時代,慢和謹慎,比快和聰明重要得多。
...
這也是為什麼兩者一開始那麼容易被混為一談——因為使用者常常只看到表面的「很多個 AI 在同時做事」,沒意識到其中有些是「角色」(長期存在、有職責分工)、有些是「分身」(一次性、執行效率用)。看清這一層後,你對 AI 輔助工作的理解會升級一個層次。

## 為什麼 Agent Team 是治理,不是工具?

最後我想花一點篇幅談一個容易被忽略的觀念:Agent Team 的最大價值,其實不在於讓你當下的工作做得更好。**它的最大價值在於「你未來可以審計這份工作」**。
...
這對個人工作者可能只是方便,對企業環境卻是**合規要求**。ISO 27001 有明確的條文要求「操作程序必須文件化」、「變更必須有審查記錄」、「職責必須分離」。Agent Team 的設計,幾乎是為這些條文量身訂做的——角色定義就是職責分離,禁止清單就是變更控制,PROJECT_STATUS.md 就是操作文件化。

還有一個價值更值得講:**Agent Team 是跨 AI 可攜的**。你把角色定義寫在 Markdown 檔案裡,不是綁在 Claude Code 這個工具上。明天你想換用其他 AI 助理、換用別的工具,只要那個工具支援讀取系統 prompt,你的 Agent Team 設計可以直接搬過去。Sub-agent 是 Claude Code 特有的機制,換工具就沒了;Agent Team 是你**自己設計的工作流**,它是你的資產,不是工具供應商的。

這個差異,對於長期投資 AI 輔助工作流的人非常重要。你投入的時間不應該綁死在單一工具上。把精力花在「寫一個好的 Agent Team 設計」,比花在「掌握某個 AI 工具的所有祕技」,投資報酬率高得多。

想像一下十年後的你。十年後,今天的 Claude Code 早就被更新的工具取代了很多代。但如果你今天寫下的 `reviewer.md`、`implementer.md` 這些定義還在,只要把它們 copy 到新工具的對應目錄,你的工作流就能延續。**這是你創造的、不會過時的工程資產**。
...
你用 Claude Code 越深入,帳單可能越驚人。Anthropic 的 API 是按 token 計費的,一個設計不當的工作流,月底拿到帳單會讓人心跳加速。有趣的是,大多數人以為「用得多就是貴」是宿命,其實不是——**token 消耗是一門可以學習的工程學**。
...
一個完整的 Agent Team 需要四根骨幹:角色定義、禁止操作清單、交接協定、狀態檔案。少一根就會垮。
...
最後,Agent Team 是**你的資產**,不是工具的功能。它跨 AI 可攜,是十年後你還能用的工程設計。
